...
export const schema = z.object({
  id: z.number(),
  header: z.string(),   // Page Title
  type: z.string(),     // Path
  source: z.string(),   // Top Source
  status: z.string(),   // Done = Active, In Process = Inactive, Deleted = Archived
  target: z.string(),   // Visitors
  limit: z.string(),    // Bounce Rate
  reviewer: z.string(), // Avg Time
})
...
const columns: ColumnDef<z.infer<typeof schema>>[] = [
  {
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <Button
          variant="link"
          className="w-fit px-0 text-left text-foreground hover:no-underline font-medium font-medium"
      ...
      <form
        onSubmit={(e) => {
          ...
          toast.promise(new Promise((resolve) => setTimeout(resolve, 1000)), {
            loading: `Saving ${row.original.header} metrics`,
            success: "Done",
            error: "Error",
          })
        }}
  ...
  {
    accessorKey: "limit",
    header: () => <div className="w-full text-right">Bounce Rate</div>,
    cell: ({ row }) => (
      <form
        onSubmit={(e) => {
          e.preventDefault()
...
export function AnalyticTable({
  ...
  const handleSave = (updatedItem: z.infer<typeof schema>) => {
    setData((prev) =>
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      onDelete: (id: number) => {
        ...
        toast.success("Page analytics archived")
      },
      onRestore: (id: number) => {
        setData((prev) =>
          prev.map((item) => (item.id === id ? { ...item, status: "Done" } : item))
        )
        toast.success("Page analytics restored as active")
      },
    },
  })
        ...
        <div className="flex items-center justify-between px-4 lg:px-6">
          <TabsList className="flex **:data-[slot=badge]:size-5 **:data-[slot=badge]:rounded-full **:data-[slot=badge]:bg-muted-foreground/30 **:data-[slot=badge]:px-1">
            <TabsTrigger value="all">
              All Analytics <Badge variant="secondary">{data.filter(d => d.status !== "Deleted").length}</Badge>
            </TabsTrigger>
            ...
            <TabsTrigger value="deleted">
              Archived <Badge variant="secondary">{data.filter(d => d.status === "Deleted").length}</Badge>
            </TabsTrigger>
          </TabsList>
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => toast.info("Google Analytics tracking is configured automatically for newly added pages.")}
            >
              <PlusIcon />
              <span className="hidden lg:inline">Add Tracker</span>
            </Button>
          </div>
        </div>
        <TabsContent
...
function TableCellViewer({ item, open, onOpenChange, mode, onSave }: TableCellViewerProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onOpenChange(false)
    toast.success(`Page analytics for "${header}" updated successfully`)
  }
    ...
    <Drawer open={open} onOpenChange={onOpenChange} direction={isMobile ? "bottom" : "right"}>
      <DrawerContent>
        <DrawerHeader className="gap-1">
          <DrawerTitle>{header || "Page Traffic Details"}</DrawerTitle>
          <DrawerDescription>
            {mode === "edit" ? "Edit Page Traffic Details & Metrics" : "Page Traffic Details & Metrics"}
          </DrawerDescription>
        </DrawerHeader>
        <div className="flex flex-col gap-4 overflow-y-auto px-4 text-sm">
            ...
            <>
              ...
              <div className="grid gap-2">
                ...
                <div className="text-muted-foreground">
                  Showing traffic and visitor distribution stats over the last 6 months.
                </div>
              </div>
              <Separator />
            </>
          ...
          <form id="drawer-post-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-3">
              <Label htmlFor="header">Page Title</Label>
              <Input
                id="header"
                value={header}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-3">
                <Label htmlFor="source">Top Traffic Source</Label>
                <Select value={source} onValueChange={setSource} disabled={mode === "view"}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-3">
                <Label htmlFor="target">Unique Visitors</Label>
                <Input
                  id="target"
                  value={target}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-3">
                <Label htmlFor="limit">Bounce Rate</Label>
                <Input
                  id="limit"
                  value={limit}
              ...
              <div className="flex flex-col gap-3">
                <Label htmlFor="reviewer">Avg. Duration</Label>
                <Input
                  id="reviewer"
                  value={reviewer}
